...
export function parseShare(value: unknown) {
  const body = object(value);
  if (Object.keys(body).some(key => !["amountCents", "expectedAmountCents", "revision"].includes(key)))
    throw new BillError(400, "Submit only your own share.");
  return { amount: cents(body.amountCents), revision: parseRevision(body),
    expectedAmount: body.expectedAmountCents === null ? null : cents(body.expectedAmountCents) };
}
export function parseEdit(value: unknown) {
  ...
  const { requestId: _request, ownShareCents: _share, ...input } = parseBill({
    ...details, requestId: "00000000-0000-4000-8000-000000000000", ownShareCents: 0,
  });
...
async function complete(tx: Tx, bill: typeof bills.$inferSelect) {
  ...
  if (bill.canceledAt || shares.some((share) => share.amountCents === null || share.confirmedAt === null)) return;
...
function currentRevision(bill: typeof bills.$inferSelect, revision: number) {
  if (bill.revision !== revision)
    throw new BillError(409, "This bill changed. Review the latest bill before trying again.");
  if (bill.canceledAt) throw new BillError(409, "This bill is canceled.");
}
async function clearConfirmations(tx: Tx, id: string) {
  await tx.update(billShares).set({ confirmedAt: null }).where(eq(billShares.billId, id));
}
export async function changeBill(id: string, userId: string, action: "edit" | "reopen" | "cancel", revision: number, input?: ReturnType<typeof parseEdit>) {
  await db.transaction(async tx => {
    const bill = await lockedBill(tx, id, userId);
    if (bill.initiatorId !== userId) throw new BillError(403, "Only the initiator can change this bill.");
    currentRevision(bill, revision);
    if (action === "cancel") {
      if (bill.completedAt) throw new BillError(409, "Reopen this bill before canceling it.");
      await tx.update(bills).set({ canceledAt: new Date(), revision: bill.revision + 1 }).where(eq(bills.id, id));
      return;
    }
    if (action === "reopen" && !bill.completedAt) throw new BillError(409, "This bill is already open.");
    if (input) {
      if (!input.participantIds.includes(userId)) throw new BillError(400, "The initiator must remain a participant.");
      const members = await tx.select().from(groupMembers).where(eq(groupMembers.groupId, bill.groupId));
      if (input.participantIds.some(id => !members.some(m => m.userId === id)))
        throw new BillError(400, "Every participant must belong to this group.");
      const shares = await tx.select().from(billShares).where(eq(billShares.billId, id));
      for (const share of shares) if (!input.participantIds.includes(share.userId))
        await tx.delete(billShares).where(and(eq(billShares.billId, id), eq(billShares.userId, share.userId)));
      for (const userId of input.participantIds) if (!shares.some(s => s.userId === userId))
        await tx.insert(billShares).values({ billId: id, userId });
    }
    await clearConfirmations(tx, id);
    await tx.update(bills).set({
      ...(input ? { title: input.title, notes: input.notes, purchaseDate: input.purchaseDate, totalCents: input.totalCents } : {}),
      completedAt: null, adjustmentCents: null, revision: bill.revision + 1,
    }).where(eq(bills.id, id));
  });
}
export async function submitShare(id: string, userId: string, input: ReturnType<typeof parseShare>) {
  await db.transaction(async tx => {
    const bill = await lockedBill(tx, id, userId);
    const [share] = await tx.select().from(billShares).where(and(eq(billShares.billId, id), eq(billShares.userId, userId)));
    if (!share) throw new BillError(403, "Only selected participants can submit a share.");
    ...
    if (share.amountCents !== input.expectedAmount)
      throw new BillError(409, "Your share changed. Review the latest bill before trying again.");
    if (bill.completedAt) throw new BillError(409, "Ask the initiator to reopen this bill before editing.");
    const changed = share.amountCents !== null && share.amountCents !== input.amount;
    if (changed) {
      await clearConfirmations(tx, id);
      await tx.update(bills).set({ revision: bill.revision + 1, adjustmentCents: null, completedAt: null }).where(eq(bills.id, id));
    }
    await tx.update(billShares).set({ amountCents: input.amount, confirmedAt: changed ? null : new Date() })
