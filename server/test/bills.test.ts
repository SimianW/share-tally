...
test('Azure tax rates freeze exactly while receipt tax allocation remains complete', async () => {
  ...
  const cases = [
    { name: 'recorded', details: [recorded], expected: { taxCents: 3, taxableBaseCents: 50 }, label: 'T (6%)' },
    { name: 'decimal', details: [{ rate: 0.13, description: 'HST' }], expected: { taxCents: 13, taxableBaseCents: 100 }, label: 'HST (13%)' },
    { name: 'no description', details: [{ rate: 0.13 }], expected: { taxCents: 13, taxableBaseCents: 100 }, label: 'Tax (13%)' },
    { name: 'zero printed rate', details: [{ rate: 0, description: 'Exempt' }], expected: { taxCents: 0, taxableBaseCents: 1 }, label: 'Exempt (0%)' },
    { name: 'no rate', details: [], expected: { taxCents: 5, taxableBaseCents: 300 }, label: undefined },
    { name: 'several rates', details: [{ rate: 0.06, description: 'GST' }, { rate: 0.07, description: 'PST' }],
      expected: { taxCents: 5, taxableBaseCents: 300 }, label: undefined },
    { name: 'same rate repeated', details: [{ rate: 0.13, description: 'HST' }, { rate: 0.13 }],
  ...
  for (const scenario of cases) {
    ...
    const delta = scenario.label ? scenario.expected.taxCents : 5;
    const denominator = scenario.label ? scenario.expected.taxableBaseCents : 300;
    const halfUp = (numerator: number) => Math.floor((2 * numerator + denominator) / (2 * denominator));
    assert.equal(corrected.items[0].allocatedTaxCents, original[0] + halfUp(delta * 200) - halfUp(delta * 100), scenario.name);
