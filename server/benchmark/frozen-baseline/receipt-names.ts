...
export async function interpretReceiptNames(
  ...
  const response = await request(`${config.baseURL}/responses`, {
    ...
    body: JSON.stringify(buildReceiptNameRequest(items, config, context)),
  });
  if (!response.ok)
    throw new Error(`Receipt name service returned HTTP ${response.status}.`);
  return parseReceiptNameResponse(items, await response.json(), context);
}

/** Exact frozen provider payload; apiKey is never serialized. */
export function buildReceiptNameRequest(items: SourceItem[], config: NameProviderConfig, context?: ReceiptContext) {
  return {
      model: config.model,
      instructions:
        (context
...
/** Exact frozen structured-response parser and correlation checks. */
export function parseReceiptNameResponse(items: SourceItem[], rawEnvelope: unknown, context?: ReceiptContext) {
  const envelope = z
    .object({
      output: z.array(
        z.object({
