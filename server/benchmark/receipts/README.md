...
This is the image and ground-truth part of [#49](https://github.com/SimianW/share-tally/issues/49), supporting the field-by-field release gate in [#47](https://github.com/SimianW/share-tally/issues/47). These files are **not** recorded Azure evidence. The offline replay harness is documented in [the benchmark README](../README.md); it consumes separately recorded provider responses. No paid API was called to build this dataset.
...
5. Remove the chosen empty entry from `owner-slots/manifest.json` and put its completed provenance entry in a new `owner/manifest.json` array, with its PNG and label JSON beside that manifest. `owner-slots/` is reserved for empty slots and is skipped by replay/recording discovery. Keep source/licence URLs resolvable (for an owner photo, use the eventual repository image/licence URLs). Run validation with the new expected counts, for example `--expected-receipts 21 --expected-owner-slots 2` after filling one slot.
6. Record Azure/model responses in #50 using the [owner-only live recording command](../README.md#explicitly-live-recording-command) and #48's evidence format. Record against the **committed redacted image**, so redacted names/card data never enter public response fixtures. Do not backfill these JSON labels from provider predictions.

## Recording boundary (#50)

The replay harness uses the raw-analysis shape from #48 (`receipt_evidence.analysis`). Pair each committed image/label ID with separately recorded Azure analyses for default, `locale=en`, and `ocrHighResolution`; model recordings are separate and adapter/version-specific because current and new model contracts differ. No public-source image here has a provider recording yet. Missing recordings are incomplete coverage, not a passing release gate. The dataset-integrity command above validates labels/images only; it is not the pipeline regression gate. See [the benchmark README](../README.md) for the offline replay contract and owner recording instructions.
