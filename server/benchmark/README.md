...
The committed-data run is expected to report **INCOMPLETE**: all 60 public Azure analyses and staged model responses still need genuine owner recording. Do not use `--allow-incomplete` as the production release gate.

## Recording layout for the owner (#50)

Record against the **committed, redacted PNG**, never the original source photo. Confirm the PNG's SHA-256 matches its source manifest before submitting it. One analysis per image/configuration:
