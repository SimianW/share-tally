...
The gate has a five-minute timeout, the review job a sixty-minute timeout, and the reviewer a forty-five-minute target budget. The action may pull its Docker image before executing the credential check.
...
The upstream action has no separate guideline-check timeout input. Its current timeout implementation gives auxiliary checks a ten-minute runway from launch, or five minutes after the main review finishes, whichever ends later, capped by the remaining overall budget and reserves for verification and posting. Queueing can consume that runway, so allowing concurrent sessions avoids the previous one-session bottleneck. This is not a guaranteed ten minutes of provider execution or an extra ten minutes after the main review. The target budget was raised from thirty to forty-five minutes after finding verification on a large PR ran out of the remaining budget; the job timeout leaves fifteen minutes for image pull and posting. See the [upstream timeout implementation](https://github.com/pgup-ai/jbot-review/blob/main/src/shared/time-budget.ts).
