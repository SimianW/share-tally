...
`deploy/nginx.conf` permits 12 MB JSON uploads and waits 180 seconds for extraction. The image limit is 8 MB before server normalization; supported formats are JPEG, PNG and WebP. Azure requests have a 120-second timeout. The background name/tax request has a 20-second timeout and is not retried. Scan logs contain Azure submit, poll, mapping and model durations plus the outcome, never receipt contents. `deploy/.env.production.example` lists the required provider settings. Credentials remain server-side. The development gateway must receive its own key rather than an implicit production OpenAI key.
