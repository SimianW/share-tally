...
Status: product decisions confirmed by the owner and published as [GitHub issue #26](https://github.com/SimianW/share-tally/issues/26). This is a specification, not an implementation report. The GitHub issue is the original requirements record. The owner-approved tax workflow amendment below supersedes its names-only boundary.

## Tax workflow amendment, 2026-09-16

The owner approved one processing action for OCR, friendly names and initial tax classification. Receipt markers take priority over model suggestions. Review exposes a boolean checkbox with human correction. This amendment records that approval locally alongside implementation.
...
3. Use Azure Document Intelligence `prebuilt-receipt`, API version `2024-11-30`, to extract receipt items and amounts. Use `gpt-5.6-luna` to interpret product names and suggest tax applicability within the same processing action. In development, name interpretation uses the configured OpenAI-compatible provider at `http://dev-2a1m:8317/v1`. Keep all provider credentials on the server. Gemini is comparison evidence, not the selected production extraction provider.
...
5. Give each item a short plain-English name using Luna. Initial processing combines extraction, friendly naming and tax-applicability suggestions. The model may suggest applicability when no usable receipt marker exists, but must not change item identities, quantities or monetary amounts. Use `Unclear Item` when the product cannot be identified. Preserve the original OCR text separately and allow viewers to reveal it. The initiator may edit the friendly name. If Luna times out or fails, keep successful Azure extraction and show the original item text with a name-service-unavailable message. Allow retry, manual naming or initiation without waiting for Luna; name interpretation is not a prerequisite for initialization.
...
- Provide editable receipt-wide and per-item discounts. The initiator enters discounts Azure did not extract. Display the difference between item costs and the actual paid total, but never interpret that difference automatically as a discount. Luna may also suggest initial tax applicability, never monetary amounts.
...
- Use explicit tax applicability and item-specific charges or discounts where recognizable or entered by the initiator. Use recognizable receipt markers first, then model suggestions. Present a Taxable checkbox, checked for taxable and unchecked for not taxable. If neither source is usable, default to taxable with a warning. Allow correction before initiation.
